...
function list_my_policies($pdo, $chat_id, $person, $bot_token) {
    ...
    $statusFa = ['REGISTERED' => 'ثبت شده', 'AWAITING_DOCS' => 'در انتظار بارگذاری مدارک', 'DOCS_PENDING' => 'در انتظار اصلاح', 'DOCS_REVIEW' => 'در انتظار تایید',
...
function send_policy_file($chat_id, $caseRow, $bot_token) {
    if ($caseRow['status'] !== 'ISSUED' || !$caseRow['issued_file_path']) {
        bale_api('sendMessage', ['chat_id' => $chat_id, 'text' => "هنوز فایل بیمه‌نامه‌ای برای این پرونده بارگذاری نشده است."], $bot_token);
        return false;
    }
    $absPath = dirname(__DIR__) . '/' . $caseRow['issued_file_path'];
    if (!file_exists($absPath)) {
        bale_api('sendMessage', ['chat_id' => $chat_id, 'text' => "❌ خطا: فایل بیمه‌نامه روی سرور پیدا نشد. لطفاً به کارشناس اطلاع دهید."], $bot_token);
        return false;
    }
    ...
    if ($curlErr || !($result['ok'] ?? false)) {
        $errDetail = $curlErr ?: ($result['description'] ?? 'خطای نامشخص');
        bale_api('sendMessage', ['chat_id' => $chat_id, 'text' => "❌ ارسال فایل بیمه‌نامه با خطا مواجه شد. لطفاً دوباره تلاش کنید یا به کارشناس اطلاع دهید."], $bot_token);
        error_log("send_policy_file failed: {$errDetail}");
        return false;
    }
    return true;
}
...
function handle_private_message($pdo, $message, $chat_id, $incoming_message_id, $text,
    ...
    if ($text === '/start' && $person) {
        ...
        if ($hasIntro) {
            ...
            send_msg($chat_id, "به منوی اصلی بازگشتید.", $bot_token, limited_menu_kb());
        }
        return;
    }
    ...
    if ($text === '/start' || !$session) {
        ...
        $premiumKb = null;
        $siteUrlForPremium = get_site_url($pdo);
        if ($siteUrlForPremium) {
            $premiumKb = ['inline_keyboard' => [[
                ['text' => '💰 استعلام حق بیمه (بدون نیاز به ورود)', 'web_app' => ['url' => $siteUrlForPremium . '/webapp/premium/index_premium.html']],
            ]]];
        }
        send_msg($chat_id, "سلام! به سامانه بیمه اتومبیل پاسارگاد ماموت خوش آمدید. 🐘\nلطفاً کد ملی خود را وارد کنید:", $bot_token, $premiumKb ?: kb_remove());
        return;
    }
